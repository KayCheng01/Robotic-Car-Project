...
#include "imu.h"
#include "config.h"
#include "barcode.h"   // barcode_init(), gpio_isr_handler(...), decoded_barcode_char

// ===== Helpers / Tiny PID (from your Demo 1) =====
typedef struct {
    float kp, ki, kd;
...
// ===== Barcode ISR fan-out (kept tiny) =====
static void gpio_shared_isr(uint gpio, uint32_t events){
    if (gpio == IR_SENSOR_PIN){
        // Hand off to your barcode ISR (in barcode.c)
        gpio_isr_handler(gpio, events);
    }
}
static void init_barcode_irq(void){
    // Both edges: measure black<->white durations
    gpio_set_irq_enabled_with_callback(
        IR_SENSOR_PIN,
        GPIO_IRQ_EDGE_RISE | GPIO_IRQ_EDGE_FALL,
        true, &gpio_shared_isr
    );
}

// ===== Drive Task (same Demo 1 logic) =====
static void vDriveTask(void *pvParameters) {
    ...
    // Capture initial heading (filtered)
    float filt_hdg = 0.f;
    for (int i = 0; i < 20; ++i) {
    ...
    for (;;) {
        ...
        // (3) Split v,w into L/R speed targets (cm/s)
        ...
        // (6) Straightness PI using R-L mismatch
        ...
        // (9) Non-blocking barcode readout (set by barcode.c after *<char>*)
        if (decoded_barcode_char != INVALID_CHAR) {
            char ch = decoded_barcode_char;
            decoded_barcode_char = INVALID_CHAR;   // ready for next scan
            printf("[BARCODE] %c\n", ch);
        }
...
int main(void) {
    ...
    // Barcode: init & IRQs on GPIO26
    barcode_init();      // sets up GP26 input, semaphore, and decoder task
    init_barcode_irq();  // enable both-edge IRQ and route to shared ISR
